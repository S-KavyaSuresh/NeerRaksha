...
export default function SimulationPage() {
  ...
  return <><p className="prototype-disclaimer">This is prototype playback using sample flood states. HEC-RAS execution is not yet connected.</p>
    ...
    <div className="form-actions"><button className="action-button" onClick={start} disabled={run.status==='running'}><Play size={16}/> Run Prototype Simulation</button>{run.status==='running'&&<button onClick={cancelRun}><Square size={15}/> Cancel</button>}<button onClick={resetRun}><RotateCcw size={15}/> Reset</button></div>{error&&<p role="alert" className="field-error">{error}</p>}
    <details className="technical-log"><summary>Selected parameters & technical status</summary><dl className="workspace-values"><div><dt>Breach type</dt><dd>{scenario.breach_type}</dd></div>{scenarioFields.map(field=><div key={field.key}><dt>{field.label}</dt><dd>{scenario[field.key]}</dd></div>)}</dl><ul><li>Data validation checks prototype parameter ranges.</li><li>Terrain preparation stages the existing map context; no terrain analysis runs.</li><li>HEC-RAS is skipped: no solver process or connection exists.</li><li>Processing selects four prebuilt sample flood frames.</li><li>Visualization starts 0–60 minute sample playback on completion.</li><li>Elapsed time measures this six-second UI workflow, not solver runtime.</li></ul></details>
  </>
}
