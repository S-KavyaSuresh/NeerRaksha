...
export default function ScenarioStudio() {
  ...
  function change(key,value) {
    ...
    setMessage('Unsaved prototype assumptions')
  }
  function save(continueToSimulation=false) {
    ...
    setMessage(result.persistent?'Prototype scenario saved on this browser origin.':'Prototype scenario saved for this session; browser storage unavailable.')
    if(continueToSimulation) navigate('/simulation')
  }
  return <><p>Editable prototype assumptions. These inputs do not recalculate the prebuilt flood states.</p>
    ...
    <form className="scenario-form" onSubmit={event=>{event.preventDefault();save()}} noValidate>
      ...
      <div className="form-actions"><button className="action-button" type="submit"><Save size={15}/> Save prototype scenario</button><button type="button" className="action-button" onClick={()=>save(true)}>Continue to Simulation <ArrowRight size={15}/></button></div>
