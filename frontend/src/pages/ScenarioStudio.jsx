...
export default function ScenarioStudio() {
  ...
  const startRun=useDashboard(state=>state.startRun)
  ...
  function save(runAfterSave=false) {
    ...
    if(runAfterSave) { startRun(); navigate('/simulation') }
  }
  return <><p>Choose Partial, Major or Custom breach assumptions, then run the approximate 2D routing scenario.</p>
    <div className="segmented" aria-label="Scenario presets">{[['partial','Partial Breach'],['major','Major Breach'],['custom','Custom']].map(([key,label])=><button key={key} aria-pressed={draft.preset===key} onClick={()=>{setDraft({...scenarioPresets[key]});setErrors({});setMessage('Preset loaded. Run simulation to apply it.')}}>{label}</button>)}</div>
    <form className="scenario-form" onSubmit={event=>{event.preventDefault();save()}} noValidate>
      ...
      <div className="form-actions"><button className="action-button" type="submit"><Save size={15}/> Save scenario</button><button type="button" className="action-button" onClick={()=>save(true)}><Play size={15}/> Run Simulation</button></div>
    </form><p className="form-status" role="status">{message}</p><div className="context-note">Partial and Major breach presets drive different propagation speed, inundation width and depth in the Ujjani routing prototype.</div>
  </>
}
