...
export function useIntelligence() {
  ...
  useEffect(() => {
    ...
    fetchDashboard(controller.signal).then(result => setData({ ...result, loading: false, fallback: false, error: '' })).catch(error => {
      ...
      setData({ summary: sampleSummary, timeline: sampleTimeline, loading: false, fallback: true, error: error.response?.data?.error?.message || 'API unavailable. Displaying local demo data.' })
    })
