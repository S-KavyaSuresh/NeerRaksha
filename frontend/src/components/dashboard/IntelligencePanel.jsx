...
export default function IntelligencePanel({ data }) {
  ...
  return <aside className={`intelligence glass ${expanded ? 'sheet-expanded' : ''}`} aria-label="Scenario intelligence">
    ...
    <div className="panel-scroll"><div className="eyebrow"><span className="status-dot"/> SCENARIO INTELLIGENCE <ArrowUpRight size={15}/></div><h2>{scenario.name}</h2><div className="risk-line"><span className="critical-badge"><ShieldAlert size={13}/> {summary.frame.risk.toUpperCase()} RISK</span><span>SYNTHETIC</span></div>
    ...
    <p className="sample-note">Synthetic simulation estimates from prebuilt frames; not GIS-derived exposure or an operational advisory.</p></div>
  </aside>
}
