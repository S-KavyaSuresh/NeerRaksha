...
export default function Timeline() {
  const { minute, playing, speed, setMinute, setSpeed, togglePlaying, restart, tick, scenario } = useDashboard()
  useEffect(() => {
    if (!playing) return
  ...
  return <section className="timeline glass" aria-label="Simulation timeline">
    <div className="timeline-heading"><span><Timer size={15} /> SIMULATION TIMELINE</span><span className="metadata">{scenario?.name || 'Ujjani breach scenario'} · approximate 2D routing</span></div>
