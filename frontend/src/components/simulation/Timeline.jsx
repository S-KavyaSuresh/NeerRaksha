...
export default function Timeline() {
  const { minute, playing, speed, setMinute, setSpeed, togglePlaying, restart, tick, selectedStudyCase } = useDashboard()
  useEffect(() => {
    if (!playing) return
  ...
  return <section className="timeline glass" aria-label="Simulation timeline">
    <div className="timeline-heading"><span><Timer size={15} /> SIMULATION TIMELINE</span><span className="metadata">{selectedStudyCase?.case_id === 'ujjani' ? 'Synthetic simulation — not yet spatially aligned' : 'Limited dataset · 60-minute view'}</span></div>
